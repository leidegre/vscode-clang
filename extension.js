...
function activate(context) {
  ...
  /** @type {(e: vscode.TextDocument) => void} */
  function didSaveTextDocument(e) {
    ...
    args = args.concat(
      conf
        .get("system.cpppath")
        .map((dir) => `-isystem "${path.normalize(substituteVariable(dir))}"`)
    );
    args = args.concat(
      conf
        .get("cpppath")
        .map((dir) => `-I"${path.normalize(substituteVariable(dir))}"`)
    );
    args = args.concat(conf.get("cxxopts"));

    var command = `clang ${args.join(" ")} \"${fn}\"`;

    var ch = createOutputChannel();
    ch.clear();
            ...
            if (d) {
              ...
              if (d.ranges.length > 0) {
                for (let k in d.ranges) {
                  pushDiagnostic(
                    path3,
                    new vscode.Diagnostic(d.ranges[k], message, severity)
                  );
                }
              } else {
                pushDiagnostic(
                  ...
                  new vscode.Diagnostic(
                    new vscode.Range(d.start, d.start),
                    message,
                    severity
                  )
                );
        ...
        for (let k in diagnosticOrigin) {
          if (diagnosticOrigin[k] === fn) {
            diagnosticCollection.delete(
              vscode.Uri.file(path.join(vscode.workspace.rootPath, k))
            );
          }
        }
        // ..and self
        diagnosticCollection.delete(
          vscode.Uri.file(path.join(vscode.workspace.rootPath, fn))
        );
...
/** @type {(value: string, workspaceFolder: string) => string} */
function substituteVariable(
  ...
  if (value) {
    return value.replace(
      VARIABLE_REGEXP,
      /** @type {(match: string, variable: string) => string} */ (
        match,
        variable
      ) => {
